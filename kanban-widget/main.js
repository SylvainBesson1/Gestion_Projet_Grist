...
window.addEventListener('load', async (event) => {
    ...
    const lookup_details = `Si vide, le widget utilise les propriétés de la colonne (basées sur les choix ou références) pour faire la liste. Sinon, vous pouvez soit :
• Fournir une liste, séparée par ";"
• Fournir une référence à une table ou une colonne commençant par "$" ($TableID ou $TableID.ColumnID)`;
    ...
    W.configureOptions(
        [
            ...
            WidgetSDK.newItem('rotation', true, 'Incliner les cartes', 'Si coché, les cartes sont inclinées aléatoirement.', 'Affichage'),
            WidgetSDK.newItem('compact', false, 'Mode compact', 'Si coché, utilise un rendu compact.', 'Affichage'),
            WidgetSDK.newItem('readonly', false, 'Lecture seule', 'Si coché, le Kanban est en lecture seule.', 'Affichage'),
            WidgetSDK.newItem('hideedit', false, 'Masquer le formulaire d\'édition', 'Si coché, masque le formulaire d\'édition lors du clic sur une carte.', 'Affichage'),
            WidgetSDK.newItem('gristeditcard', false, 'Carte Grist', 'Si coché, ouvre la carte Grist en double-cliquant.', 'Affichage')
        ],
        '#config-view',
        '#main-view',
...
/* -------------------------------------------------
   GESTION DES CHANGEMENTS DE MAPPING
------------------------------------------------- */

...
function updateFilterUI() {
    ...
    projBtn.className = 'filter-btn' + (filters.projet ? ' active' : '');
    prioBtn.className = 'filter-btn' + (filters.priorite ? ' active' : '');
    etatBtn.className = 'filter-btn' + (filters.etat ? ' active' : '');
    ...
    if (filters.projet) {
        ...
        chips.push(`<span class="filter-chip" style="border-left:3px solid ${col}" title="${escapeHtml(proj[options.projectNameCol])}">
            ${escapeHtml(proj[options.projectNameCol].length > 15 ? proj[options.projectNameCol].substring(0, 15) + '…' : proj[options.projectNameCol])}
...
function getFilteredTasks() {
    return taches.filter(task => {
        // Projet
        if (filters.projet) {
            const projId = Number(filters.projet);
            if (Number(task[options.projectCol]) !== projId) return false;
        }
        // Priorité
